...
class GpuCollector:
    ...
    def sample(self) -> list[dict[str, Any]]:
        try:
            ...
            self._last_error = f"NVML sampling failed: {exc}"
            raise RuntimeError(self._last_error) from exc
...
def _sample_gpu(index: int, timestamp: str) -> dict[str, Any]:
    handle = pynvml.nvmlDeviceGetHandleByIndex(index)
    memory = pynvml.nvmlDeviceGetMemoryInfo(handle)
    utilization = _read_utilization(handle)
    return {
        "timestamp": timestamp,
        "gpu_index": index,
...
def _read_utilization(handle: Any) -> dict[str, float | None]:
    try:
        utilization = pynvml.nvmlDeviceGetUtilizationRates(handle)
        return {"gpu": float(utilization.gpu), "memory": float(utilization.memory)}
